...
export default function Sidebar({ user = { name: "Doris", address: "0x7e83...4a2c" }, mode = "individual" }) {
  ...
  const visibleNavItems = navItems.filter(
    (item) => item.showFor === "all" || item.showFor === mode
  );
  ...
  return (
    <aside className={styles.sidebar}>
      ...
      <div className={styles.brand}>
        ...
        <div className={`${styles.modeBadge} ${isCorporate ? styles.modeCorporate : styles.modeIndividual}`}>
          {isCorporate ? (
            <>
      ...
      <div className={styles.userSection}>
        ...
        <div className={styles.userInfo}>
          ...
          <span className={styles.userWallet}>{user.address}</span>
        </div>
      </div>
    </aside>
  );
